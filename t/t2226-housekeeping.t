...
# Usage: list_jobs
list_jobs () {
	flux module stats job-manager | jq -r '.housekeeping.running | keys[]'
}

# Usage: kill_all signum
kill_all () {
	flux python -c "import flux; print(flux.Flux().rpc(\"job-manager.housekeeping-kill\",{\"signum\":$1}).get())"
}

# Usage: kill_job jobid signum
kill_job () {
	flux python -c "import flux; print(flux.Flux().rpc(\"job-manager.housekeeping-kill\",{\"signum\":$2, \"jobid\":$1}).get())"
}

# Usage: kill_ranks idset signum
kill_ranks () {
	flux python -c "import flux; print(flux.Flux().rpc(\"job-manager.housekeeping-kill\",{\"signum\":$2, \"ranks\":\"$1\"}).get())"
}
