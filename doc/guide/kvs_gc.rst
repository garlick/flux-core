...
- **content-backing.gc-info** — return :math:`\text{current_epoch}` (which
  flux-gc freezes as the horizon :math:`H`) and :math:`\text{COUNT}(\text{epoch}
  < H)` for a given threshold. flux-gc uses only ``current_epoch``; the count
  is not a meaningful "reclaimable" estimate before the mark phase runs, since
  it includes reachable data whose epoch the mark would refresh.
...
- It is **one frozen value, not two**, which is simpler to reason about and to
  get right.
- It is **more aggressive**: because it marks reachable blobs all the way up to
  :math:`H`, it can safely sweep below :math:`H`. The per-checkpoint variant
  marks only to :math:`e_i` and so must sweep below the lower :math:`e_{\text{old}}`
  to avoid deleting reachable data — leaving unreachable garbage in
  :math:`[e_{\text{old}}, H)` uncollected until checkpoint pruning raises
  :math:`e_{\text{old}}` past it. The single horizon reclaims that band
  immediately.
...
dedup re-store, or a mark). Apply this to each run in turn: whatever :math:`A`
sweeps is reachable from no live root, and :math:`B`'s roots are live roots, so
:math:`A`'s sweep cannot touch anything :math:`B` relies on. The same holds
with :math:`A` and :math:`B` exchanged, and neither statement assumes an
ordering between the two runs. Two further facts close the concurrent case:
...
- **Cached subtree traversal.** This is *not* the per-run visited set GC
  ...
  epochs (see "Why one horizon instead of per-checkpoint epochs"): an
  already-marked subtree is exactly one whose treeobj is stamped at or above
  its checkpoint's epoch.
