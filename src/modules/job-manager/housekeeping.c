...
static void bulk_complete (struct bulk_exec *bulk_exec, void *arg)
{
    ...
    allocation_remove (a, zlistx_cursor (a->hk->allocations));
}
...
static json_t *housekeeping_get_stats_job (struct allocation *a)
{
    char *s;
    json_t *job;
    double now = flux_reactor_now (flux_get_reactor (a->hk->ctx->h));

    if (!(s = idset_encode (a->pending, IDSET_FLAG_RANGE)))
        return NULL;
    job = json_pack ("{s:f s:s s:i}",
                     "t_start", a->t_start,
                     "pending", s,
                     "runtime", (int)(now - a->t_start));
    free (s);
    return job;
}
