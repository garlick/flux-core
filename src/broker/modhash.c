                                   ...
                                   flux_error_t *errp)
{
    ...
    for (i = 0; i < ARRAY_SIZE (loaders); i++) {
        ...
        if (asprintf (&pattern, "%s%s", target, loaders[i].glob) < 0)
            return errprintf (errp, "failed to construct module name glob");
        files = dirwalk_find (searchpath,
                              DIRWALK_REALPATH | DIRWALK_NORECURSE,
                              pattern,
                              1,
                              NULL,
                              NULL);
        if (files && zlist_size (files) != 0) {
            if (!(info.path = strdup (zlist_first (files)))) {
                ERRNO_SAFE_WRAP (free, pattern);
                ERRNO_SAFE_WRAP (zlist_destroy, &files);
                return errprintf (errp, "failed to duplicate module path");
            }
            info.loader = loaders[i].cmd;
        }
        zlist_destroy (&files);
        free (pattern);
        if (info.path)
    ...
    if (!info.path) {
        ...
        errno = ENOENT;
        return errprintf (errp, "module not found in search path");
    }
    if (!(info.name = strdup (name_override ? name_override : target))) {
        modinfo_release (&info);
        return errprintf (errp, "failed to duplicate module name");
    }
    *infop = info;
    return 0;
};
 ...
 * On success, fill 'infop' (caller must call modinfo_release()) and return 0.
 * On failure, fill errp and return -1.
 * N.B. The responsibility to make sure the file exists and conforms to RFC 5
 * is the loader's responsibility, not this function.
 */
int modhash_resolve_bypath (const char *target,
                            const char *name_override, // may be NULL
                            struct modinfo *infop,
                            flux_error_t *errp)
{
    ...
    for (int i = 0; i < ARRAY_SIZE (loaders); i++) {
        char *pattern;
    ...
    if (!info.loader) {
        modinfo_release (&info);
        errno = EINVAL;
        return errprintf (errp, "could not determine loader from module path");
    }
    *infop = info;
    return 0;
}
                     ...
                     void *arg)
{
     ...
     */
         ...
         */
        if (fnmatch ("*.so*", info.path, 0) == 0 && !exec)
