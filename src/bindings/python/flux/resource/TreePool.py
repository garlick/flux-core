...
def _dedup_topology(groups_by_rank):
    ...
    for rank, (levels, topo) in groups_by_rank.items():
        key = tuple(tuple(group for group in level) for level in levels)
...
class _TreePoolV1(Rv1Pool):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Sub-instance resource modules re-rank allocated nodes from 0.
        # Map original children rank keys to pool-rank space via sorted-zip.
    ...
    def _copy_extra(self, new):
        ...
        new._type_levels = self._type_levels
        new._type_topo = self._type_topo
        new._rank_type = self._rank_type
    ...
    def _container_exclusive_select(self, candidates, request):
        ...
        for rank, info, free_cores, free_gpus in candidates:
            if nslots_target is not None and allocated_slots >= nslots_target:
                break
            topo = self._rank_topo(rank)
            groups = _groups_for_container(topo, container_level)
            free_groups = [
            ...
            max_take = (
                ...
                else len(free_groups)
            )
            take_groups = free_groups[:max_take]
            alloc_cores = frozenset().union(*(gc for gc, _ in take_groups))
            alloc_gpus = frozenset().union(*(gg for _, gg in take_groups))
            selected.append((rank, alloc_cores, alloc_gpus))
            allocated_slots += len(take_groups)
