...
struct marker {
    ...
    int marked;         // total blobs whose epoch was raised (for stats)
 ...
 * which flux-gc freezes as the horizon H.  The gc-info RPC also returns a
 * count of blobs below a threshold epoch, but that is not used here: before
 * the mark phase runs it counts reachable data too, so it is not a meaningful
 * estimate of what a sweep would reclaim.
 */
static int gc_info (flux_t *h, int64_t *current_epoch)
{
...
static int mark_all_roots (flux_t *h, json_t *roots_array, int *markedp)
{
    struct marker m = { .h = h };
    size_t index;
    json_t *root;
...
static int sweep_blobs (flux_t *h, int *deletedp)
{
    int total_deleted = 0;
...
int cmd_gc (optparse_t *p, int argc, char **argv)
{
     ...
     */
    log_msg ("gc complete: marked %d, reclaimed %d blobs (horizon epoch %jd)",
             marked,
             deleted,
