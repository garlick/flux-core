...
#   - checkpoint churn (checkpoints advance H and prune old ones)
#   - concurrent GC runs
#
...
#
# Checkpoints are what advance the GC horizon H and, once more than
# content-sqlite.max_checkpoints exist, prune the oldest.  The KVS creates a
# checkpoint only when kvs.checkpoint-period is configured (its default is 0.0,
# i.e. the periodic checkpoint is disabled) or when a FLUX_KVS_SYNC commit is
# made.  So rather than depend on instance configuration for the checkpoint-
# churn hazard, this test drives it directly with periodic FLUX_KVS_SYNC commits
# (the CheckpointChurn activity), quickly exceeding max_checkpoints so that
# pruning of old checkpoints is exercised on a default instance.
#
...
class JobChurn(Activity):
    ...
    def step(self):
        ...
        flux.job.wait(self.handle, jobid)
        time.sleep(0.01)


class CheckpointChurn(Activity):
    """Force checkpoints so the GC horizon H advances and old ones are pruned.

    A FLUX_KVS_SYNC commit flushes and checkpoints the primary root.  This
    drives the checkpoint-churn hazard without relying on the instance having
    kvs.checkpoint-period configured (its default is disabled).
    """

    def step(self):
        # Change the root, then checkpoint it: FLUX_KVS_SYNC makes this commit
        # flush and checkpoint (what "flux kvs sync" does), so no separate sync.
        flux.kvs.put(self.handle, "soak.checkpoint.tick", self.iterations)
        flux.kvs.commit(self.handle, flags=flux.constants.FLUX_KVS_SYNC)
        time.sleep(0.1)
...
def main():
    ...
    activities = [
        ...
        CheckpointChurn(stop, args.verbose),
    ]
